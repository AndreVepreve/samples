...
```latex
\[
\displaystyle P(A)=\sum_i P(A\mid B_i)\,P(B_i)
\]
```

$$\displaystyle P(A)=\sum_i P(A\mid B_i)\,P(B_i)$$

for a partition \(\{B_i\}\). This is how we combine per-segment conversion rates into an overall rate, or marginalize latent variables. 

**Bayes’ rule.** In classification and filtering, we invert conditionals (“likelihoods”) into posteriors:
```latex
\[
\displaystyle P(H\!\mid D) \propto P(D\!\mid H)\,P(H)
\]
```

$$\displaystyle P(H\!\mid D) \propto P(D\!\mid H)\,P(H)$$

This underpins Naive Bayes, spam filtering, and medical diagnosis—any time you update beliefs with new evidence. 

**(Conditional) independence.** Conditional independence assumptions let us factor high-dimensional likelihoods; e.g., Naive Bayes assumes features are conditionally independent given the class. Understanding when that’s approximately true is crucial for model bias/variance trade-offs. 
...
- **Poisson**: counts of rare events in a fixed interval; mean equals variance \(\lambda\).  
- **Exponential**: inter-arrival times with the *memoryless* property; the only continuous memoryless distribution.

**Law of rare events (sketch).** A Binomial(\(n,p\)) with \(n\to\infty,\,p\to 0\), \(np=\lambda\) tends to Poisson(\(\lambda\)), explaining why Poisson fits rare-event phenomena. 
...
```latex
\[
\displaystyle \Pr(|X-\mu|\ge k\sigma)\le \frac{1}{k^2}.
\]
```

$$\displaystyle \Pr(|X-\mu|\ge k\sigma)\le \frac{1}{k^2}.$$

Use when you want distribution-free tail bounds.

---

## 4) Sampling & Asymptotics (Why “averaging works”)

- **Law of Large Numbers (LLN):** sample averages converge to the true mean as \(n\) grows—why empirical rates stabilize with more data.  
- **Central Limit Theorem (CLT):** the standardized mean is approximately Normal (under mild conditions), enabling z/t intervals and tests even when the population isn’t Normal.
...
```latex
\[
\displaystyle H(p,q)= -\mathbb E_{p}\,[\log q]
\]
```
...
```latex
\[
\displaystyle H(p,q)=H(p)+D_{\mathrm{KL}}(p\Vert q),
\]
```

$$\displaystyle H(p,q)=H(p)+D_{\mathrm{KL}}(p\Vert q),$$

minimizing cross-entropy with fixed \(p\) is equivalent to minimizing **KL divergence** from truth to model. In one-hot classification, cross-entropy reduces to negative log-likelihood (MLE).

**Engineering note (stability).** Compute softmax/log-loss with the **log-sum-exp** trick (subtract max-logit before exponentiating) to avoid overflow/underflow.
...
SciPy documents `ttest_ind` and the Welch option.
...
The API explains coverage properties and trade-offs.
...
SciPy’s `bootstrap` implements percentile and BCa intervals.
...
Small \(p\) suggests shift.

**Population Stability Index (PSI)** quantifies covariate shift via bucketed divergence; common monitoring thresholds: \(\approx 0.1\) moderate, \(\approx 0.2\) severe (rules of thumb vary by org).
```python
import numpy as np
...
1. **Don’t trust Wald CIs for proportions.** Prefer Wilson/Clopper–Pearson in small-n/extreme-p regimes.  
2. **ROC vs PR.** On imbalanced data, PR is often more informative than ROC for selection decisions.  
3. **Welch over Student’s t** with unequal variances; when in doubt, permutation test your chosen statistic.  
4. **Power before you test.** Compute minimal detectable effect (MDE) & \(n\) up-front.  
5. **Monitor drift.** Add KS and PSI checks to your pipeline; alert on sustained deviations.  
6. **Calibrate if you need probabilities.** Add reliability diagrams & Brier score to evaluation.
